...
export function getSeverity(score: number | null | undefined): SeverityInfo {
  ...
  if (score >= 9)  return { label: "Critical", cls: "bg-[#ef4444]/15 text-[#f87171] border-[#ef4444]/30", textCls: "text-[#f87171]", barCls: "bg-[#ef4444]", dotCls: "bg-[#ef4444]" };
...
export function severityFilterToParams(filter: string): Record<string, string> {
  switch (filter) {
    case "critical": return { severity_min: "9" };
    case "high":     return { severity_min: "7", severity_max: "8.99" };
