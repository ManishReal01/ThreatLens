...
export function getSeverity(score: number | null | undefined): SeverityInfo {
  ...
  if (score >= 8.0)  return { label: "Critical", cls: "bg-red-500/20 text-red-400 ring-1 ring-red-500/30",          textCls: "text-red-400",    barCls: "bg-red-500",    dotCls: "bg-red-400" };
  if (score >= 6.5)  return { label: "High",     cls: "bg-orange-500/20 text-orange-400 ring-1 ring-orange-500/30", textCls: "text-orange-400", barCls: "bg-orange-500", dotCls: "bg-orange-400" };
  if (score >= 4.0)  return { label: "Medium",   cls: "bg-amber-500/20 text-amber-400 ring-1 ring-amber-500/30",    textCls: "text-amber-400",  barCls: "bg-amber-500",  dotCls: "bg-amber-400" };
...
export function severityFilterToParams(filter: string): Record<string, string> {
  switch (filter) {
    case "critical": return { severity_min: "8" };
    case "high":     return { severity_min: "6.5", severity_max: "7.99" };
    case "medium":   return { severity_min: "4", severity_max: "6.49" };
